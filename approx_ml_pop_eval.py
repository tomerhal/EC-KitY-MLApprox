...
class ApproxMLPopulationEvaluator(PopulationEvaluator):
    ...
    population_sample_size : int, optional
        number of individuals to sample and compute their fitness when approximating , by default 10
        ...
        for sub_population in population.sub_populations:
            if self.should_approximate():
                ...
                if self.gen > 0 and self.gen % self.gen_sample_step == 0:
                    ...
                    sample_inds = random.sample(sub_population.individuals, self.population_sample_size)
