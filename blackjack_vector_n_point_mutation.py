...
class BlackjackVectorNPointMutation(FailableOperator):
    """
    ...
    def attempt_operator(self, individuals, attempt_num):
        ...
        for individual in individuals:
            ...
            m_points = choices(range(0, individuals.size()), weights=individuals[0].get_rewards(), k=self.k)
...
class BlackjackFloatVectorUniformOnePointMutation(BlackjackVectorNPointMutation):
    """
...
class BlackjackFloatVectorUniformNPointMutation(BlackjackVectorNPointMutation):
    """
...
class BlackjackFloatVectorGaussOnePointMutation(BlackjackVectorNPointMutation):
    """
    ...
    def on_fail(self, payload):
        ...
        """
        mut = BlackjackFloatVectorUniformNPointMutation(1, self.probability, self.arity, self.events)
        return mut.apply_operator(payload)


class BlackjackFloatVectorGaussNPointMutation(BlackjackVectorNPointMutation):
    """
    ...
    def on_fail(self, payload):
        ...
        Handle gauss mutation failure by returning a callable uniform mutation
        """
        mut = BlackjackFloatVectorUniformNPointMutation(self.n, self.probability, self.arity, self.events)
        return mut.apply_operator(payload)
